...
public class EditCourseJSONRequestController implements JSONRequestController {
  ...
  public void process(JSONRequestContext requestContext) {
    ...
    rowCount = requestContext.getInteger("componentsTable.rowCount").intValue();
    for (int i = 0; i < rowCount; i++) {
      ...
      if (componentId == -1) {
        componentId = courseDAO.createCourseComponent(course, componentLength, componentTimeUnit, componentName,
            componentDescription).getId();
      }
      else {
        courseDAO.updateCourseComponent(courseDAO.getCourseComponent(componentId), componentLength, componentTimeUnit,
